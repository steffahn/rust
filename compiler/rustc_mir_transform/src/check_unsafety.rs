        ...
        match rvalue {
            Rvalue::Aggregate(box ref aggregate, _) => match aggregate {
                ...
                &AggregateKind::Closure(def_id, _) | &AggregateKind::Generator(def_id, _, _) => {
                    ...
                    self.register_violations(violations.iter(), used_unsafe_blocks.iter());
                }
            },
            _ => {}
        }
...
impl<'tcx> UnsafetyChecker<'_, 'tcx> {
    fn require_unsafe(&mut self, kind: UnsafetyViolationKind, details: UnsafetyViolationDetails) {
        ...
        self.register_violations(
            iter::once(&UnsafetyViolation { source_info, lint_root, kind, details }),
            iter::empty(),
        );
    }
    ...
    fn register_violations<'a>(
        ...
        match safety {
            ...
            Safety::Safe => violations.for_each(|&violation| {
            ...
            Safety::FnUnsafe => violations.for_each(|&(mut violation)| {
...
fn check_unused_unsafe(
    ...
    let context = match tcx.hir().find(hir_id) {
        Some(Node::Item(&hir::Item { kind: hir::ItemKind::Fn(ref sig, _, _), .. }))
            if sig.header.unsafety == hir::Unsafety::Unsafe =>
        {
            Context::UnsafeFn(hir_id)
        }
        _ => Context::Safe,
    };
    ...
    let mut unused_unsafe = vec![];

    let mut visitor =
        UnusedUnsafeVisitor { tcx, used_unsafe_blocks, context, unused_unsafe: &mut unused_unsafe };
...
fn unsafety_check_result<'tcx>(
    ...
    let unused_unsafe = (!tcx.is_closure(def.did.to_def_id()))
...
pub fn check_unsafety(tcx: TyCtxt<'_>, def_id: LocalDefId) {
    ...
    let UnsafetyCheckResult { violations, unused_unsafe, .. } = tcx.unsafety_check_result(def_id);
    ...
    for &(block_id, kind) in unused_unsafe.as_ref().unwrap() {
